...
export function renderPreviewView(container, state, handlers) {
  ...
  container.innerHTML = `
    <section class="px-4 pb-6 sm:px-6 animate-fadeIn">
...
function renderVideoInfo(info) {
  return `
    <div class="flex gap-4">
      ${info.thumbnail ? `
        <div class="thumbnail-wrapper h-24 w-40">
      ...
      ` : `
        <div class="thumbnail-wrapper thumbnail-placeholder h-24 w-40">
          <i class="fas fa-film text-2xl"></i>
        </div>
      `}
      <div class="min-w-0 flex-1">
        <h3 class="line-clamp-2 font-semibold" style="color: var(--text-primary)">${escapeHtml(info.title)}</h3>
        ${info.uploader ? `
          <p class="mt-1 text-xs flex items-center gap-1.5" style="color: var(--text-secondary)">
            <i class="fas fa-user text-[10px]" style="color: var(--text-tertiary)"></i>${escapeHtml(info.uploader)}
          </p>
        ` : ''}
        ${info.duration ? `
          <p class="mt-1 text-sm flex items-center gap-1.5" style="color: var(--text-secondary)">
...
function renderQualitySelector(state) {
  return `
    <div class="mt-6">
      <label class="text-sm font-semibold" style="color: var(--text-secondary)">${t('app.select_quality')}</label>
  ...
  return `
    <div class="mt-6">
      <div class="flex items-center justify-between mb-2">
        <span class="text-sm font-semibold" style="color: var(--primary)">${t('app.downloading')}</span>
        <span class="text-sm font-bold" style="color: var(--text-secondary)">${percent}%</span>
      </div>
      ...
      ${state.speed || state.eta ? `
        <div class="flex items-center justify-between mt-2 text-xs" style="color: var(--text-tertiary)">
          ${state.speed ? `<span><i class="fas fa-tachometer-alt mr-1"></i>${escapeHtml(state.speed)}</span>` : ''}
          ${state.eta ? `<span><i class="fas fa-clock mr-1"></i>${escapeHtml(state.eta)}</span>` : ''}
        </div>
      ` : ''}
    </div>
  `;
...
function renderSuccess(state) {
  return `
    <div class="mt-6 success-banner">
      <div class="success-icon">
        <i class="fas fa-check"></i>
      </div>
      <div>
        <p class="font-bold" style="color: var(--success)">${t('app.download_complete')}</p>
        ${state.filename ? `<p class="text-sm mt-0.5 truncate" style="color: var(--text-secondary)">${escapeHtml(state.filename)}</p>` : ''}
      </div>
    </div>
  `;
}
